...
namespace Heron\Core\Model\Config\Structure;

use Magento\Config\Model\Config\Structure\Data as StructureData;
use Heron\Core\Block\Adminhtml\System\Config\Button;
use Heron\Core\Block\Adminhtml\System\Config\Docs;
use Heron\Core\Block\Adminhtml\System\Config\Form\Field\Version;
use Heron\Core\Block\Adminhtml\System\Config\Message;
use Heron\Core\Helper\Validate as Helper;

/**
 * Plugin to add 'Module Information' group to each modules (before general group)
 *
 * Class Data
 * @package Heron\Core\Model\Config\Structure
 */
class Data
{
    /**
    ...
    public function beforeMerge(StructureData $object, array $config)
    {
        ...
        foreach ($sections as $sectionId => $section) {
            if (isset($section['tab']) && ($section['tab'] === 'Heron') && ($section['id'] !== 'Heron')) {
    ...
    protected function getFieldList()
    {
        return [
            ...
            'name'        => [
                ...
                'frontend_class' => 'Heron-module-active-field-free Heron-module-active-name',
                'show'           => Helper::MODULE_TYPE_FREE
            ],
            'email'       => [
                'label'          => __('Register Email'),
                'validate'       => 'required-entry validate-email',
                'frontend_class' => 'Heron-module-active-field-free Heron-module-active-email',
                'comment'        => __('This email will be used to create a new account at Heron.com, Heron help desk (to get priority support).'),
                'show'           => Helper::MODULE_TYPE_FREE
            ],
            'product_key' => [
                'label'          => __('Product Key'),
                'frontend_class' => 'Heron-module-active-field-key',
                'show'           => 'WEAGERJHGWEWRGHSETH' //Helper::MODULE_TYPE_FREE
            ],
            'button'      => [
